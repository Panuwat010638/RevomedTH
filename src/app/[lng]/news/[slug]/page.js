...
export async function getStaticParams({ params, searchParams }, parent) {
    ...
    if(lng == "th"){
        ...
        const pathsCN = [] = await client.fetch(
            `*[_type == "newCN" && defined(slug.currenct)][].slug.current`
        )
...
async function getPosts(params) {
    ...
    if(lng == "th"){
        ...
        const query = groq`*[_type == "newsCN" && slug.slug.current == '${slug}'][0]{
            title,
            mainImage,
            body,
            date,
          ...
          const queryBlog = groq`*[_type == "newsCN" && slug.slug.current != '${slug}' && category->title == "${posts?.category}"] | order(_createdAt desc){
            title,
            mainImage,
            body,
            date,
